                ...
                if(isset($_POST['update_click'])){//cap nhap
                    ...
                    if($error == false && !empty($_POST['quantity'])){                       
                        ...
                        foreach ($oderProducts as $key => $product) {
                            $insertString .= "(NULL, '".$orderID."', '".$product['prd_id']."', '".$_POST['size'][$product['prd_id']]."', '".$_POST['quantity'][$product['prd_id']]."', '".$product['price']."', '".time()."', '".time()."')";
                            if($key != count($oderProducts) - 1){
                                $insertString.= ",";
                            }
                        }   
                        $insertoder = mysqli_query($connect,"INSERT INTO `oder_detail` (`id`, `id_oder`, `id_prd`, `size`, `quantity`, `price`, `created_time`, `last_updated`) VALUES ".$insertString." ");
                        $succsess = header('location:http://localhost/btl/admin/index.php');
...
<body>
    ...
    <section class="cart">
        ...
        <div class="succsess"><?=(!empty($succsess)) ? $succsess : ""?>
        <div class="succsess-con"><p>Đặt hàng thành công</p>
            <div class="abc"><p>V</p></div>
            <a href="http://localhost/btl/cartegory.php"><button style ="background-color:#3379e7;border:none;height:40px;padding:0 6px 0 6px;border-radius:2px;color:#fff;cursor: pointer">Tiếp tục mua sắm</button></a>
        </div>
    </div>
